...
- **README.md** - テスト情報更新、API詳細追加（フィルタリングパラメータ、ページングレスポンス形式）
- **frontend-admin/README.md** - プロジェクト専用のドキュメントに刷新
- **frontend-public/README.md** - プロジェクト専用のドキュメントに刷新
- **CONTRIBUTING.md** - コントリビューションガイド新規作成（TDD、コーディング規約、PR作成ガイド）
- **PROJECT_TEMPLATE.md** - TDDワークフロー、品質重視テストガイドラインを追加

### 品質テスト拡充

Domain層とApplication層の単体テストを大幅に拡充：
- 不変条件テスト（ID一意性、タイムスタンプ整合性）
...
`/api/members` エンドポイントで `groupId`, `generation`, `isGraduated` フィルタが動作するように修正。
...
#### テスト
- [x] Application Handlers テスト追加（9個 → 30個完了）
- [x] Domain テスト品質向上（80個 → 149個完了）
- [ ] frontend-admin 単体テスト（React Testing Library）
- [ ] E2Eテスト（Playwright）

**現在のテスト状況（2025-12-31）:**
- Domain テスト: 149件（不変条件、境界値、エラーハンドリング、ビジネスルール、状態遷移）
- Application テスト: 30件（ページング、フィルタリング、DTO変換）
- 合計: 179件
