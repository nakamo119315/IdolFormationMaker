using IdolManagement.Application.Formations.DTOs;
using IdolManagement.Domain.Formations.Entities;
using IdolManagement.Domain.Formations.Repositories;

namespace IdolManagement.Application.Formations.Commands;

public record CreateFormationCommand(CreateFormationDto Dto);

public class CreateFormationHandler
{
    private readonly IFormationRepository _formationRepository;

    public CreateFormationHandler(IFormationRepository formationRepository)
    {
        _formationRepository = formationRepository;
    }

    public async Task<FormationDto> HandleAsync(CreateFormationCommand command, CancellationToken cancellationToken = default)
    {
        var formation = Formation.Create(
            command.Dto.Name,
            command.Dto.GroupId
        );

        foreach (var posDto in command.Dto.Positions)
        {
            var position = FormationPosition.Create(
                formation.Id,
                posDto.MemberId,
                posDto.PositionNumber,
                posDto.Row,
                posDto.Column
            );
            formation.AddPosition(position);
        }

        await _formationRepository.AddAsync(formation, cancellationToken);

        return ToDto(formation);
    }

    private static FormationDto ToDto(Formation formation) => new(
        formation.Id,
        formation.Name,
        formation.GroupId,
        formation.Positions.Select(p => new FormationPositionDto(
            p.Id, p.MemberId, p.PositionNumber, p.Row, p.Column)),
        formation.CreatedAt,
        formation.UpdatedAt
    );
}
